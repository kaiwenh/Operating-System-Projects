...
ospfs_dir_readdir(struct file *filp, void *dirent, filldir_t filldir)
{
	...
	uint32_t offset; //indicate where we are reading
...
allocate_block(void)
{
  ...
  void *ptr = ospfs_block(OSPFS_FREEMAP_BLK);

  int i;
  for(i = OSPFS_FREEMAP_BLK + 1; i < size; i++)
    {
      if(bitvector_test(ptr, i) == 1)
	{
	  bitvector_clear(ptr, i);
	  return i;
	}
    }
  return 0;
...
{
	/* EXERCISE: Your code here */
  uint32_t block_num = ospfs_size2nblocks(ospfs_super->os_ninodes * OSPFS_INODESIZE) 
			+ ospfs_super->os_firstinob;
  
  if(blockno <= ospfs_super->os_nblocks - 1 && blockno > block_num)
...
indir_index(uint32_t b)
{
  ...
  if(b < OSPFS_NDIRECT)
    ...
    {
      eprintk("Error: block number is too big!");
      return 0;
    }
}
...
static void
clean_block(uint32_t block)
{
  int i;
  char *ptr = ospfs_block(block);
...
add_block(ospfs_inode_t *oi)
{
	...
	if(n >= OSPFS_MAXFILEBLKS)
	  return -ENOSPC; // Replace this line
	...
	if (indir2 != -1)
	  {
		...
		if (indir == 0)
		  {	
		    tmp_dir1 = allocate_block();
		    if(tmp_dir1 == 0)
		      {
			free_block(new_block);
			return -ENOSPC;
		      }
		    clean_block(tmp_dir1);
		    oi->oi_indirect2 = tmp_dir1;
		  }
		
		indir2_ptr = ospfs_block(oi->oi_indirect2);
		tmp_dir2 = allocate_block();
		if(tmp_dir2 == 0)
		  {
		    free_block(new_block);
		...
		clean_block(tmp_dir2);
		indir2_ptr[indir] = tmp_dir2;
	      }

	    indir2_ptr = ospfs_block(oi->oi_indirect2);
	    indir_ptr = ospfs_block(indir2_ptr[indir]);
	    indir_ptr[dir] = new_block;
	  }
...
{
	// current number of blocks in file
       uint32_t n = ospfs_size2nblocks(oi->oi_size)-  1; //transfer from 1 based to 0 based
	...
	if(indir2 == -1)
	  ...
	  {
	    uint32_t *indir_ptr2 = ospfs_block(oi->oi_indirect2);
	    uint32_t *indir_ptr = ospfs_block(indir_ptr2[indir]);
	...
	if(ospfs_size2nblocks(oi->oi_size) < ospfs_size2nblocks(new_size))
	  {
	    ...
	    //shrink to orignal file if error occurs in growth.
	    if(r < 0)
	      {
...
create_blank_direntry(ospfs_inode_t *dir_oi)
{
  ...
  for(offset = 0; offset < dir_oi->oi_size; offset += OSPFS_DIRENTRY_SIZE)
    {
      ptr = ospfs_inode_data(dir_oi, offset);
  ...
  if(add_block(dir_oi) < 0)
    ...
    return ospfs_inode_data(dir_oi, offset);// Replace this line
}
...
ospfs_link(struct dentry *src_dentry, struct inode *dir, struct dentry *dst_dentry) {
	/* EXERCISE: Your code here. */
  ospfs_inode_t *oi = ospfs_inode(dir->i_ino);


  if(dst_dentry->d_name.len > OSPFS_MAXNAMELEN)
    return -ENAMETOOLONG;
  
  if(find_direntry(oi, dst_dentry->d_name.name, dst_dentry->d_name.len))
    return -EEXIST;
  
  ospfs_direntry_t *entr = create_blank_direntry(oi);

    if(IS_ERR(entr))
        return PTR_ERR(entr);
    
    ospfs_inode_t *dst = ospfs_inode(src_dentry->d_inode->i_ino);
    ...
    entr->od_ino = src_dentry->d_inode->i_ino;
    
    memcpy(entr->od_name, dst_dentry->d_name.name, dst_dentry->d_name.len);
    
    entr->od_name[dst_dentry->d_name.len] = '\0';
...
ospfs_create(struct inode *dir, struct dentry *dentry, int mode, struct nameidata *nd)
{
    ...
    ospfs_direntry_t *dst = create_blank_direntry(dir_oi);
    ...
    memcpy(dst->od_name, dentry->d_name.name, dentry->d_name.len);
    
    dst->od_name[dentry->d_name.len] = '\0';
    
    en->oi_size = 0;
    
    en->oi_ftype = OSPFS_FTYPE_REG;
...
ospfs_symlink(struct inode *dir, struct dentry *dentry, const char *symname)
{
	...
	ospfs_direntry_t *entr = create_blank_direntry(dir_oi);
	...
	ospfs_symlink_inode_t *sym_link = ospfs_inode(i);
	sym_link->oi_size = strlen(symname);
	sym_link->oi_ftype = OSPFS_FTYPE_SYMLINK;
	memcpy(sym_link->oi_symlink, symname, strlen(symname));
	
	sym_link->oi_symlink[strlen(symname)]='\0';

	
	entr->od_ino = i;
	memcpy(entr->od_name, dentry->d_name.name, dentry->d_name.len);
	entr->od_name[dentry->d_name.len] = '\0';
...
{
	ospfs_symlink_inode_t *tmp_oi =
		(ospfs_symlink_inode_t *) ospfs_inode(dentry->d_inode->i_ino);
	// Exercise: Your code here.

	nd_set_link(nd, tmp_oi->oi_symlink);
	return (void *) 0;
}
...
MODULE_AUTHOR("Wenhao Yu, Kaiwen Huang");
